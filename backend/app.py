...
with gr.Blocks() as app:
    config: dict = data.get('title')
    gr.HTML(f"""<h1 style="color: darkgreen;">{config.get('company_name')}</h1>""")
    bank_name = gr.Textbox(label="Название аудироемого лица", value=config.get('bank_name'))
    ...
    boss_name = gr.Textbox(label="Руководитель проверки", value=config.get('boss_name'))
    with gr.Tabs():
...
Просьма избегать сумм и прочих выводов внизу.
        ...
        with gr.TabItem("807 форма"):
            from form_807 import process
            gr.Markdown("## Ключевое поле: *рабочая колонка*")

            with gr.Row():
                file_input = gr.File(
                    label="Загрузите Excel файл (.xlsx)", file_types=[".xlsx"]
                )
                filter_start = gr.Textbox(label="Цифры начала", value="706")
                sheet_input = gr.Textbox(label="Название листа", value="Приложение_ОСВ")
                main_col_name = gr.Textbox(label="С какой колонкой работать", value="рабочая колонка")
